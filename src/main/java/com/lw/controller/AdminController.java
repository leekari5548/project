...
//            String userId = jo.getString("user_id");
    ...
    @RequestMapping("person/status/update")
    public String changeStatus(String request_data) {
        try {
            JSONObject jo = JSON.parseObject(request_data);
            adminService.changeStatus(jo);
            return ResponseUtils.sendRes(0, "成功", null);
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            return ResponseUtils.sendRes(-1, "系统错误", "");
    ...
    @RequestMapping("/group/match/get")
    public String groupMatch(String request_data) {
        try {
            JSONObject jo = JSON.parseObject(request_data);
            return adminService.updateMatch(jo);
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            return ResponseUtils.sendRes(-1, "系统错误", "");
        }
    ...
    @RequestMapping("group/match/status")
    public String status(String request_data) {
        try {
            JSONObject jo = JSON.parseObject(request_data);
            Boolean b = adminService.matchGet(jo);
            JSONObject res = new JSONObject();
            res.put("match", b);
            return ResponseUtils.sendRes(0, "success", res);
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            return ResponseUtils.sendRes(-1, "系统错误", "");
        }
    ...
    public String delSubject(String request_data) {
        try {
            JSONObject jo = JSON.parseObject(request_data);
            adminService.removeSubject(jo);
    ...
    public String addMatch(String request_data){
        JSONObject jo = JSONObject.parseObject(request_data);
        adminService.insertMatch(jo);
        return ResponseUtils.sendRes(0, "success", null);
    }
    @RequestMapping("match/get")
    public String getMatch(){
        return ResponseUtils.sendRes(0,"success",adminService.getMatches());
    }

    @RequestMapping("group/can_match")
    public String canMatchGroup(){
        try {
            return ResponseUtils.sendRes(0,"success",adminService.getCanSelect());
        }catch (Exception e){
            logger.error(e.getMessage(), e);
            return ResponseUtils.sendRes(-1, "系统错误", "");
        }
