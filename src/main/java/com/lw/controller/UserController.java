...
//            System.err.println(list);
    ...
    @RequestMapping("/user/reset/verify")
    public String verify(String request_data){
        try {
            logger.info(request_data);
            JSONObject jo = JSON.parseObject(request_data);
            boolean b = userService.verify(jo);
            if (b){
                return ResponseUtils.sendRes(0,"success","");
            }else {
                return ResponseUtils.sendRes(-1,"验证失败","");
            }
        }catch (Exception e){
            logger.error(e.getMessage(),e);
            return ResponseUtils.sendRes(-1,"系统错误","");
    ...
    public String resetPassword(String request_data){
        try {
            logger.info(request_data);
            JSONObject jo = JSON.parseObject(request_data);
            userService.updatePassword(jo);
            return ResponseUtils.sendRes(0,"success","");
        }catch (Exception e){
            logger.error(e.getMessage(),e);
            return ResponseUtils.sendRes(-1,"系统错误","");
