...
public interface LoginMapper {
    ...
    List<Person> getPersonByType(@Param("type") String type, @Param("groupId") String groupId,@Param("status")String status);

    int updateNum(Person person);

    int updateStatus(@Param("username") String username,@Param("status") String status);


    int checkUser1(@Param("username") String username,@Param("idCard") String idCard);
